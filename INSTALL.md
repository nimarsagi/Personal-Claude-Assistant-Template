...
~/.claude/CLAUDE.md — so it carries its own, stricter safety gate. Run this
once, when this folder is first placed at its permanent path (e.g.
~/my-claude-assistant/). Re-running is safe (see idempotency check below);
it does not delete itself, since re-runs may legitimately be needed if the
user ever moves or renames the folder.
...
2. **Backup before any edit — both files, unconditionally.** Copy the
   ...
   ~/.claude/backup-<YYYY-MM-DD-HHMMSS>/, even if settings.json is not
   being changed in this step. "I'm only editing one file" is not an
   exemption — back up both, every time this file runs. Use a full
   timestamp, not just the date: a date-only folder name means a second
   run on the same day overwrites the first backup with whatever is
   on disk at that moment — which, if an edit already happened earlier
   that day, means the pristine "before" copy gets silently clobbered by
   an already-edited copy, destroying the one snapshot you'd want to
   revert to. A timestamped folder makes every run's backup immutable and
   independent of how many times this file has run before. If
   ~/.claude/CLAUDE.md does not exist yet, note that explicitly instead
...
4. **Append-only, delimited, capped.** Append (never rewrite or reorder
   ...
   comments so it's unambiguous to find and to remove later:

   ```
   <!-- my-claude-assistant:start -->
   ## My Claude Assistant
   Read ~/my-claude-assistant/CLAUDE.md at the start of this session — it
   is the boot protocol for persistent memory, journal, and user model.
   Follow its instructions (including running SETUP.md if present).
   <!-- my-claude-assistant:end -->
   ```

   Adjust the path only if this folder does not live at
   ~/my-claude-assistant/ for this install.
